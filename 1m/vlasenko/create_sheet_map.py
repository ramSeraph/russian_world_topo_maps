...
def main():
    ...
    base_url = 'https://maps.vlasenko.net/soviet-military-topographic-map/map1m.html'

    try:
        with open('data/map1m.html', 'r', encoding='cp1251') as f:
            html_content = f.read()
    ...
    for link in links:
        ...
        if not sheet_id_raw or not re.match(r'^[a-zA-Z]{1,2}-\d{2}(_\d+)*$', sheet_id_raw) or not relative_url:
            continue
