...
class GSMapstorProcessor(TopoMapProcessor):
    ...
    def __init__(self, filepath, extra, index_box, index_properties, id_override=None):
        ...
        self.no_first_warp = extra.get('no_first_warp', False)
    ...
    def export_bounds_file(self):
        ...
        if self.no_first_warp:
            cutline_crs = 'EPSG:4326'
        else:
            cutline_crs = 'EPSG:4284'

        self.run_external(f'ogr2ogr -t_srs EPSG:4326 -s_srs {cutline_crs} -f GeoJSONSeq {str(bounds_file)} {cutline_file}')

        feature = json.loads(bounds_file.read_text())
        # if all the coordinates have positive longitudes, except one which is -180.0 change it to 180.0
        all_coords = feature['geometry']['coordinates'][0]
        negative_longitudes = [ coord for coord in all_coords if coord[0] < 0 ]
        print(negative_longitudes)
        if len(negative_longitudes) >= 1 and all([ n[0] == -180.0 for n in negative_longitudes ]):
            print(f'Fixing -180.0 longitude to 180.0 in bounds file {bounds_file}')
            fixed_coords = [ (180.0 if coord[0] == -180.0 else coord[0], coord[1]) for coord in all_coords ]
            feature['geometry']['coordinates'][0] = fixed_coords
            bounds_file.write_text(json.dumps(feature))
    ...
    def get_crs_proj_real(self):
        ...
        return f'+proj=tmerc +lat_0=0 +lon_0={zone_lon} +k=1 +x_0={zone_x} +y_0=0 +ellps=krass +towgs84={to_wgs84} +units=m +no_defs +type=crs'
    ...
    def georeference(self):
        ...
        if self.no_first_warp:
            #from_crs_proj = self.get_crs_proj()
            from_crs = CRS.from_epsg('4284')
    ...
    def first_warp(self):
        ...
        if self.same_proj_resolution is not None:
            res = self.same_proj_resolution
        else:
            res = self.get_same_proj_resolution()
            if res[0].imag != 0 or res[1].imag != 0: 
                raise ValueError(f"Invalid resolution calculated: {res}")
        #print(res)
        #exit(0)
        crs_proj = self.get_crs_proj_real()
        FACTOR = 0.5
    ...
    def warp(self):
        ...
        if self.no_first_warp:
            inp_file = workdir.joinpath('georef.tif')
            inp_crs_proj = 'EPSG:4326'
        else:
            self.first_warp()
            inp_file = workdir.joinpath('warped.tif')
            inp_crs_proj = self.get_crs_proj_real()
        ...
        if self.no_first_warp:
            ...
            transformer = Transformer.from_crs(CRS.from_epsg(4284), CRS.from_epsg(4326), always_xy=True)
            sheet_ibox = [ transformer.transform(p[0], p[1]) for p in sheet_ibox ]
        else:
            cutline_crs_proj = 'EPSG:4284'


        #sheet_ibox = rewind({ "type": "Polygon", "coordinates": [sheet_ibox] }, rfc7946=False)['coordinates'][0]
        ...
        size_options = ''
        reproj_options = f'{size_options} -r bilinear -t_srs "EPSG:3857" -s_srs "{inp_crs_proj}"' 
        nodata_options = '-dstalpha'
        perf_options = '-multi -wo NUM_THREADS=ALL_CPUS --config GDAL_CACHEMAX 1024 -wm 1024' 
        ...
        warp_cmd = f'gdalwarp -overwrite {perf_options} {nodata_options} {reproj_options} {warp_quality_options} {cutline_options} {str(inp_file)} {str(final_file)}'
        self.run_external(warp_cmd)
...
def process_files():
    ...
    if from_list_file is not None:
        ...
        # Find all gif files
        print(f"Finding gif files in {data_dir}")
    ...
    for filepath in image_files:
        ...
        if '_' not in id:
            print(f'Skipping {filepath} due to hyphen in id')
            continue
