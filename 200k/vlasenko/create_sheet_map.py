...
def main():
    ...
    base_url = 'https://maps.vlasenko.net/soviet-military-topographic-map/map200k.html'

    try:
        with open('data/map200k.html', 'r', encoding='cp1251') as f:
            html_content = f.read()
    except FileNotFoundError:
        print("Error: data/map200k.html not found.")
        return
    ...
    for link in links:
        ...
        if not sheet_id_raw or not re.match(r'^[a-zA-Z]-\d{2}-\d{2}(_\d+)*$', sheet_id_raw) or not relative_url:
            continue
