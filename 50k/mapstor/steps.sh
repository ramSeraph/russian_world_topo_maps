...
cat data/mapstor_ids.txt| grep "\-\-gs\-\-050k\-\-" > data/gs_50k_ids.txt

# 3. get list of zip files for each id
cat data/gs_50k_ids.txt | xargs -I {} uvx --from internetarchive ia list {} -l | grep "zip$" > data/zip_urls.txt
