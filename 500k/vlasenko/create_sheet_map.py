...
def main():
    ...
    base_url = 'https://maps.vlasenko.net/soviet-military-topographic-map/map500k.html'

    try:
        with open('data/map500k.html', 'r', encoding='cp1251') as f:
            html_content = f.read()
    except FileNotFoundError:
        print("Error: data/map500k.html not found.")
        return
    ...
    for link in links:
        ...
        print(f"Processing sheet ID: {sheet_id_raw}")
        if not sheet_id_raw or not re.match(r'^[a-zA-Z]{1,2}-\d{2}-\d{1}(_\d+)*$', sheet_id_raw) or not relative_url:
            continue
