...
def get_sheetmap():
    with open('uwm/data/sheet_map.json', 'r') as f:
        sheet_map = json.load(f)
    return sheet_map

def get_bad_sheet_ids():
    bad_sheets_file = Path('uwm/bad_sheets.txt')
...
def process_files():
    ...
    special_cases_file = Path(__file__).parent / 'uwm'/ 'special_cases.json'
